...
getcap /home/ed/ws_pi5/.venv/bin/python3
```
...
## 5b. Link up/down — `cap_net_admin` on the venv python (NOT on `/bin/ip`)

`hw_test/link.py` brings the Pi-facing NIC down and back up during
the L2 link-flap tests. The natural approach is `ip link set <iface>
down/up`, but on at least one Ubuntu 24.04 / kernel 6.17 system,
applying `cap_net_admin=eip` to `/usr/bin/ip` is silently ignored at
exec time — `getcap /usr/bin/ip` reports the cap is set, `setcap -v`
agrees, but the running process has `CapPrm=0` and `link set down`
returns `Operation not permitted`. The same cap mechanism works fine
on `tcpdump`, `ethtool`, `dumpcap`, and `arping`.

Rather than spelunk into that, `link.py` does link up/down via raw
`AF_NETLINK` from the venv python directly. That only requires
`cap_net_admin` on the venv python interpreter, which is added by
`setup-caps.sh` alongside the existing `cap_net_raw`:

```bash
sudo setcap cap_net_admin,cap_net_raw=eip /home/ed/ws_pi5/.venv/bin/python3
```

Verify with the live test in §7.

If you want to chase down why `/usr/bin/ip` ignores file caps on your
system, the diagnostic loop is: setcap, then run `/usr/bin/ip monitor &`
and check `grep ^Cap /proc/$!/status` — `CapPrm` should be `0x1000`
(cap_net_admin); if it's `0`, the kernel didn't grant the cap. None
of the L2 tests depend on `/bin/ip` having caps, so this is informational
only.
...
# Link up/down via the netlink helper:
cd hw_test && ../.venv/bin/python3 -c \
    "import link; link.link_down('enx00e04c0a2bed'); link.link_up('enx00e04c0a2bed')"
```
...
- **Package upgrades.** `sudo apt upgrade tcpdump` replaces the binary
  ...
  `wireshark-common` (which owns `dumpcap`), `ethtool`, and
  `iputils-arping`. Re-run `setcap` after any upgrade that touches
  these packages. To check what was recently touched:
